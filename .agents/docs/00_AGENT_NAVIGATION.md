...
6. **Performance Profiling:** When adding heavy operations to the execution loop, wrap them using the existing context manager (e.g., `with self.profiler.measure("feature_name"):` in `Mode_master`). You can view the profiling output (FPS, execution times) in the console by setting `"printCpuFpsInfo": true` in `config/app_config.json`, or via the Web UI's websocket `profilerMetrics` payload.
