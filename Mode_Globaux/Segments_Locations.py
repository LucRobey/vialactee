class Segments_Locations:
    def __init__(self):
        ...
        # Segment h10: from (205, 73) to (290, 73) -> Swapped to 189 to be floor-level to match physical setup
        self.segment_h10 = [[coord_x + self.len_h00, self.len_v1 + self.offset_h30_v4] for coord_x in range(self.len_h10)]
        # Segment h11: from (205, 189) to (291, 189) -> Swapped to 73 to be top-level to match physical setup
        self.segment_h11 = [[coord_x + self.len_h00, self.len_v1//3 + self.offset_h30_v4] for coord_x in range(self.len_h11)]
        ...
        # Segment v1: from (205, 16) to (205, 188) wired bottom-up so coords are reversed!
        self.segment_v1 = [[self.len_h00, coord_y + self.offset_h30_v4] for coord_y in range(self.len_v1)][::-1]
        # Segment v2: from (291, 73) to (291, 245) wired bottom-up
        self.segment_v2 = [[self.len_h00 + self.len_h10, coord_y + self.offset_h30_v4 + self.len_v1//3] for coord_y in range(self.len_v2)][::-1]
        # Segment v3: from (382, 16) to (382, 188) wired bottom-up
        self.segment_v3 = [[self.len_h00 + self.len_h10 + self.len_h20, coord_y + self.offset_h30_v4] for coord_y in range(self.len_v3)][::-1]
        # Segment v4: from (430, 32) to (430, 204) wired bottom-up
        self.segment_v4 = [[self.len_h00 + self.len_h10 + self.len_h20 + self.len_h31, coord_y + self.offset_h30_v4*2] for coord_y in range(self.len_v4)][::-1]
