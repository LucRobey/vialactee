...
class Robust_Simulated_Microphone:
    ...
    def calculate_fft(self):
        ...
        if self.raw_fft_history is not None:
            ...
            self.bass_flux = np.sum(freq_diff[1:6])
            self.treble_flux = np.sum(freq_diff[6:300])
            self.high_res_flux = self.treble_flux
        self.raw_fft_history = fft_result



def localized_continuous_phase_sweep(odf_buffer, center_bpm, search_radius=1.5, step=0.1, expected_phase=None):
...
#bass_arr = filter_window(bass_arr, start_time, end_time)
#treble_arr = filter_window(treble_arr, start_time, end_time)
treble_arr = 0.5*treble_arr
