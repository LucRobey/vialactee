...
while mic.pop_chunk(CHUNK_SIZE_FOR_60FPS):
    ...
    if len(future_queue) >= lookahead_frames:
        ...
        if present.get('is_beat', False): algorithmic_beats.append(playhead_time)
        if present.get('is_sub_beat', False): algorithmic_sub_beats.append(playhead_time)
...
def evaluate_single_stream(alg_stream, true_stream, tolerance=0.150):
    ...
    return hits, false_positives, false_negatives, precision, recall, f1_score

m_hits, m_wrongs, m_misses, m_precision, m_recall, m_f1 = evaluate_single_stream(alg_beats_array, true_beat_times, TOLERANCE_WINDOW)
s_hits, s_wrongs, s_misses, s_precision, s_recall, s_f1 = evaluate_single_stream(alg_sub_beats_array, true_subbeat_times, TOLERANCE_WINDOW)
