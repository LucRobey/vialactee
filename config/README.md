...
- **`segments_locations.json`**: Defines the 2D coordinate geometry used by transitions and spatial effects.
...
## How it works:
When the program starts, the configuration manager parses the JSON files. The `segments.json` and `segments_locations.json` geometry is used by `Segment.py` to correctly map the 1D NeoPixel array into a 2D logical workspace, ensuring the visual algorithms display correctly on the physical chandelier.
