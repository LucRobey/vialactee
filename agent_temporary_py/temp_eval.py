...
def localized_continuous_phase_sweep(odf_buffer, center_bpm, search_radius=3.0, step=0.1):
    ...
    for bpm_val in bpm_evals:
        ...
        template_vals[((abs_phi >= 0.22) & (abs_phi <= 0.28))] = 0.3
...
while mic.pop_chunk(CHUNK_SIZE_FOR_60FPS):
    ...
    if frames_since_sweep >= frames_between_sweep:
        ...
        # FIX: We MUST inject the high-resolution continuous floating phase!
        # Do a strong pull to violently override the native integer-corrupted sliding phase
        phase_diff = (target_phase - listener.beat_phase + 0.5) % 1.0 - 0.5
        listener.beat_phase += phase_diff * 0.8
        
        # Safely wrap beat count without dropping transient triggers over boundaries
        while listener.beat_phase >= 1.0:
            listener.beat_phase -= 1.0
            listener.beat_count += 1
        while listener.beat_phase < 0.0:
            listener.beat_phase += 1.0
            listener.beat_count -= 1
    ...
    if last_logged_beat_phase < 0.5 and current_phase >= 0.5:
        ...
        if getattr(listener, 'sub_beat_locked', -1) < last_logged_beat_count:
            is_sub_beat = True
            listener.sub_beat_locked = last_logged_beat_count
